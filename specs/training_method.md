...
**Curriculum replay (anti-forgetting).** Sequential phase-from-phase fine-tuning risks catastrophic forgetting of earlier perception as the model specializes on CoT. To counter this, Ph-4/Ph-5 batches mix in ~10% of earlier-phase (Ph-1–Ph-3) samples — `train.tier1.replay_ratio: 0.1` — so visual grounding and structured-ASG emission are rehearsed while later-stage reasoning is learned. This follows the staged VQA → CoT curriculum of the Skew-T VLM (`lee2025skewt`), where each stage must not regress the grounding the next stage depends on.

**Auto-generation of VQA/CoT supervision.** Ph-1 VQA and Ph-4 CoT targets are auto-generated directly from the ASG scalars via a **threshold-bucket + bag-of-phrases** scheme: peak-VIL values are bucketed into intensity bins, the growth scalar `g` into an `init`/`grow`/`decay`/`steady` regime, and motion into magnitude/direction phrases; these are composed into questions, answers, and rationale chains. This is a sharper, lower-hallucination version of the existing template → LLM-polish recipe — every clause stays anchored to an ASG attribute, so the supervision cannot assert content the state does not contain (`lee2025skewt`).
...
**Balanced reconstruction (`L_render`) — heavy-rain reweighting** (`asgwm.train.losses.balanced_weight_map` + `balanced_mse`, wired into `tier2_total_loss`): plain MSE/MAE on VIL is dominated by the light-rain majority, so skill at the high CSI thresholds (where the thesis lives) gets washed out. Following the HKO-7 benchmark (`shi2017trajgru`), the Stage-C reconstruction term is computed as a **balanced MSE** under a per-pixel weight ladder keyed to intensity. We adapt HKO-7's original rain-rate cuts `{2,5,10,30}` mm/h → weights `{1,2,5,10,30}` to our **VIL-byte** thresholds, aligned to the eval CSI thresholds (`eval.csi_thresholds_vil`):
- `balanced_loss: true`
- `balanced_thresholds: [16,74,133,181]` — 4 cut points (VIL byte) → 5 bins
- `balanced_weights: [1,2,5,10,30]` — one weight per bin (`len = thresholds + 1`); the validity mask sets invalid pixels to weight 0 (set `balanced_loss: false` to ablate).

This term **composes with, and does not replace,** the physics/realism terms — `L_mass`/`L_spectral`/`L_nonneg`/`L_continuity` all still apply; the balanced weight map only reshapes the pixel-recon contribution. Justification to cite (`shi2017trajgru`): on HKO-7, B-MSE/B-MAE training objectives correlate with CSI/HSS far better than plain MSE/MAE under Kendall-tau rank correlation, because they stop the model from collapsing toward the no-rain mean. Concretely this serves (i) the **heavy-rain CSI thresholds** in the Group-A skill table and (ii) the **Group-B initiation/growth** gain, where the relevant pixels are exactly the high-VIL ones the ladder up-weights.
...
**Pretrain-then-rollout-finetune schedule.** To fight error accumulation over long lead times, the renderer is trained in two stages, following YingLong (`xu2025yinglong`): (1) **single-step pretrain** under a normalized relative-L2 objective `||xhat - x||^2 / ||x||^2` (scale-robust across the VIL dynamic range); then (2) **rollout fine-tune** through autoregressive multi-step prediction so the model is optimized on its own compounding errors. YingLong reports an empirical **rollout-depth ceiling around `T ≈ 6`** (deeper rollout gave no further gain), so the fine-tune horizon is capped there rather than unrolled to the full lead. This maps onto our staging: the **13→12** headline (12 future steps) is trained with the short rollout, and the **13→36** long-lead configuration extends the same schedule — pretrain single-step, then rollout-finetune up to the `T ≈ 6` ceiling and chain — rather than back-propagating through all 36 steps at once. This stacks with scheduled sampling (oracle → inferred ASG) above: scheduled sampling anneals the *input-side* state error, the rollout fine-tune anneals the *output-side* recurrence error.
