...
>
> **Philosophy:** see `philosophy.md` for the unified method-framework ontology.
...
INPUT  X_{t-k..t}  = radar VIL  +  IR069/IR107  +  water-vapor  +  GLM lightning
   ...
   ▼  STAGE B — TRANSITION:  ASG_t ──► ASG_{t+h}   +   render_NL_delta(ASG_t, ASG_{t+h})  ◄ readout #2
   │            advection + continuity + growth/decay constrained ── AMBIGUITY RESOLVED HERE
   │            (NL readout #2 is a deterministic downstream render, not a freeform LM output)
   ▼  ═══════════ FAITHFUL BOTTLENECK ═══════════
...
A **small transition transformer** over ASG tokens (≈10–50 M params): object tokens + context tokens → predicted object tokens at horizon `h`. It must emit predicted positions, **merge/split/initiation/decay** events, and an updated growth field. **Readout #2** is computed *after* prediction: `render_NL_delta(ASG_t, ASG_{t+h})` — a deterministic, grammar-faithful template render of the transition (not a freeform LM output from Stage B). Three injection points for governing equations (all compute-cheap — loss terms + a warp, no extra parameters):

1. **Differentiable advection operator (Stage B).** Object centroids and the growth field are advanced by a semi-Lagrangian warp using the ASG motion field; the network predicts the *residual* on top of advection, so the linear-motion baseline is built in (mirrors the conservation idea in NowcastNet's evolution network, `zhang2023nowcastnet`).
2. **PINN-style residual losses (Stage B)** (`raissi2019pinn`): a **continuity / mass-conservation** residual on the predicted growth field and a smoothness/advection residual on the motion field, penalizing equation violation on the predicted *state*.
3. **Equation-aware training prompts (Stage A, Ph-5).** The governing relations (advection, continuity, growth–decay parameterization) are stated in the VLM curriculum system prompt so perception training grounds on equations, not prose alone. At inference, equation injection in the transition is via (1) and (2) above.
...
**NL as a downstream render — not a freeform channel.** The natural-language descriptions at readouts #1 and #2 are generated by deterministic template functions `render_NL(ASG)` and `render_NL_delta(ASG_t, ASG_{t+h})`:
