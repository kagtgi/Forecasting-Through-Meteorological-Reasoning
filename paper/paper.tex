...
\begin{abstract}
Short-range precipitation nowcasting fails most severely during convective initiation and rapid growth---precisely the regimes where a brief radar history under-constrains the future and physical reasoning is most decisive, and whose frequency is rising under climate change.
Existing deep-learning nowcasters encode meteorological knowledge as opaque latent vectors that a forecaster cannot inspect or intervene on, and physics-informed variants inject only governing equations, leaving seasonal, geographic, diurnal, and synoptic knowledge unrepresented.
We introduce the Atmospheric Scene Graph World Model (ASG-WM), in which a vision-language model reads a multi-channel radar--satellite sequence and constructs an explicit, human-readable world-model state---the Atmospheric Scene Graph (ASG)---encoding storm-object positions, motion vectors, growth and decay tendencies, regime classifications, and per-attribute uncertainty, requiring no human-supplied text at inference.
A physics-informed renderer then materialises the forecast field exclusively from this predicted state and a future-blind advection of the current frame, creating an architectural information bottleneck in which the structured state---not a decorative rationale---is the sole causal conduit to the forecast.
Faithfulness is therefore testable by intervention: by construction, zeroing the scene graph collapses the forecast to advection, and structured perturbations propagate to the rendered field as the state predicts.
Evaluated on the SEVIR benchmark against twelve baselines spanning extrapolation, recurrent, convolutional, transformer, and generative families, ASG-WM is designed to achieve competitive overall skill while improving on convective initiation and rapid growth by \emph{[TBR]} CSI at the heavy-precipitation threshold over the strongest generative competitor, at a modest, explicitly reported cost on steady-advection events.
A systematic ablation isolates five meteorological knowledge categories---governing equations, seasonal priors, geographic context, diurnal forcing, and synoptic patterns---each tested with a targeted held-out benchmark.
We release the automatic ASG labelling pipeline, which requires no GPU and no manual annotation at training scale.
\end{abstract}
...
Stage~B, a small transition transformer, rolls the ASG forward under a differentiable advection operator, PINN-style~\cite{raissi2019pinn} continuity residuals, and equation-aware prompting.
...
A human-readable description and forecast rationale are emitted alongside the state, but are derived from it, not relied upon as the computational driver.
...
\noindent\emph{All quantitative entries in this manuscript are reported as [TBR] (to be reported). This revised manuscript specifies the complete architecture, evaluation protocol, and experiment designs; numerical results will populate the tables and figures on completion of the staged compute plan described in Methods. Figures with illustrative values are labelled accordingly.}
...
CSI and HSS at three thresholds, FSS at two neighbourhood scales, pooled CSI over 0--3\,h, mean CRPS, and LPIPS.
...
\toprule
 & & \multicolumn{3}{c}{CSI ($\uparrow$)} & \multicolumn{3}{c}{HSS ($\uparrow$)} & FSS & Pooled & CRPS \\
\cmidrule(lr){3-5}\cmidrule(lr){6-8}
Method & Family & L & M & H & L & M & H & 16${\times}$16 ($\uparrow$) & CSI ($\uparrow$) & ($\downarrow$) \\
\midrule
...
\midrule
\textbf{ASG-WM (ours)} & Reasoning WM & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{\textbf{Computational footprint} (TBR).
Inference latency per event, peak inference memory, ensemble cost ($K{=}10$), and training cost relative to leading generative baselines.}
\label{tab:compute}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Method & Infer.\ latency (ms) $\downarrow$ & Peak mem.\ (GB) $\downarrow$ & Ensemble ($K{=}10$) (s) $\downarrow$ & Train (GPU-h) $\downarrow$ \\
\midrule
...
The full data-flow architecture---encoder, structured-state grammar, physics-constrained transition, symbolic admissibility check, faithful bottleneck, and rectified-flow renderer---is shown in Figure~\ref{fig:arch}.
...
(\emph{Stage~B}) a transition transformer advances the state to $\text{ASG}_{t+h}$ as a residual on advection, with governing-equation injection (advection operator, PINN continuity, equation-aware prompting), an optional symbolic admissibility check, and a forecast-rationale readout.
...
(a)~a differentiable semi-Lagrangian advection operator ($\partial\phi/\partial t + \mathbf{v}\!\cdot\!\nabla\phi = 0$);
(b)~a PINN-style mass-conservation residual ($\partial g/\partial t + \nabla\!\cdot\!(g\mathbf{v}) \approx 0$);
and (c)~equation-aware prompting that states the advection and growth-decay relations in the system prompt.
...
Stage~B simultaneously emits a natural-language forecast rationale before predicting $\text{ASG}_{t+h}$, so that the symbolic and linguistic reasoning are co-generated from the same physics-constrained dynamics.
...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item \textbf{Equation-aware prompting.} The governing relations (advection equation $\partial\phi/\partial t + \mathbf{v}\cdot\nabla\phi = 0$; mass conservation; growth-decay parameterisation as a function of CAPE, CIN, and shear) are stated in the system prompt so the VLM reasons explicitly with the equations.
\end{enumerate}

\noindent Stage B emits a second constrained NL rationale summarising the predicted evolution (regime changes, merges/splits, initiations, dissipations).
